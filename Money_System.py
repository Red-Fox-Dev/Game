class MoneySystem:
    ...
    def capture_point(self):  # ยึดจุด
        ...
        while self.is_capturing and self.capture_count < 3:  # ทำงานต่อไปจนกว่าจะครบ 3 จุด
            ...
            # เพิ่มเงื่อนไขให้ผู้เล่นเดินออกได้ตามต้องการ
            user_input = input("Do you want to stop capturing this point? (yes/no): ")
            if user_input.lower() == "yes":
                self.is_capturing = False
                messages.append("You've left the capture point.")
                break
